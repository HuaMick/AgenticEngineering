...
class TemplateWorkflow:
    ...
    def __init__(
        ...
        # Add default AgenticGuidance templates directory
        default_templates = self._find_default_templates_dir()
        if default_templates and default_templates not in self.templates_dirs:
            self.templates_dirs.append(default_templates)
    ...
    def _find_default_templates_dir(self) -> Optional[Path]:
        """Find the default templates directory in AgenticGuidance."""
        module_root = Path(__file__).parent.parent.parent.parent.parent
        candidates = [
            module_root / "assets" / "templates",
...
def create_template_context_from_project(project_root: Path) -> TemplateContext:
    """Create TemplateContext from a project root path.
    ...
    context.worktree = str(project_root)
