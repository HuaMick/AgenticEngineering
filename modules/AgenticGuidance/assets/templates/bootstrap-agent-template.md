...
| Command | Purpose |
|---------|---------|
| `agentic context bootstrap --role {{ROLE_ID}}` | Get Seed Context (objective, process, inputs) |
| `agentic context role {{ROLE_ID}}` | Get role-specific process/guidelines |
| `agentic context task` | Get active task from current plan |
| `agentic context inputs --role {{ROLE_ID}}` | Get input files manifest |
