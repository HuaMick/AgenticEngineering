...
class TestUserStoriesIntegrationWithOtherCommands:
    ...
    def test_stories_after_template_generate(self, integration_cli, integration_repo):
        """Test adding stories to a template-generated plan file.

        Verifies that user stories can be added to and read from
        a plan file generated by template generate.
        """
        # Scaffold a plan
        stdout, stderr, code = integration_cli("plan", "scaffold", "template-stories")
        assert code == 0

        plan_path = integration_repo / "docs" / "plans" / "live" / "template-stories"

        # Generate a build template
        output_file = plan_path / "plan_build.yml"
        stdout, stderr, code = integration_cli(
            "template", "generate", "build",
            "--objective", "Test stories with template",
            "--phases", "P1:Build,P2:Test",
            "--output", str(output_file)
        )
        assert code == 0

        # Read the template and add user_stories
        content = yaml.safe_load(output_file.read_text())
        content["user_stories"] = [
            {
                "id": "US-TPL-001",
                "as": "developer",
                "i_want": "use templates with stories",
                "so_that": "I have a complete plan",
            },
        ]
        with open(output_file, "w") as f:
            yaml.dump(content, f)

        # Verify stories can be listed
        stdout, stderr, code = integration_cli(
            "plan", "stories", "list",
            "--plan", str(plan_path)
        )
        assert code == 0
        assert "US-TPL-001" in stdout
