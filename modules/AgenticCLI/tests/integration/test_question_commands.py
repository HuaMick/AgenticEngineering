...
class TestQuestionAskCommand:
    ...
    @pytest.fixture
    def epic_with_questions_dir(self, temp_repo):
        """Create plan folder with questions directory."""
        plan_path = temp_repo / "docs" / "epics" / "live" / "260203QC_question_test"
    ...
    def test_ask_creates_question(self, cli_runner, epic_with_questions_dir):
        """Test creating a new question."""
        result = cli_runner([
            "agent", "question", "ask",
            "What is the expected behavior?",
            "--plan", str(epic_with_questions_dir),
            "--severity", "high"
        ])
        ...
        pending_dir = epic_with_questions_dir / "questions" / "pending"
        question_files = list(pending_dir.glob("Q-*.yml"))
        assert len(question_files) == 1

    def test_ask_with_default_severity(self, cli_runner, epic_with_questions_dir):
    ...
    def test_ask_with_json_output(self, cli_runner, epic_with_questions_dir):
...
class TestQuestionListCommand:
    ...
    @pytest.fixture
    def epic_with_questions(self, temp_repo):
        """Create plan folder with sample questions."""
        import time
        import yaml

        plan_path = temp_repo / "docs" / "epics" / "live" / "260203QC_list_test"
        plan_path.mkdir(parents=True)
    ...
    def test_list_pending_questions(self, cli_runner, epic_with_questions):
        """Test listing pending questions."""
        result = cli_runner([
            "plan", "question", "list",
            "--plan", str(epic_with_questions)
        ])
    ...
    def test_list_answered_questions(self, cli_runner, epic_with_questions):
    ...
    def test_list_all_questions(self, cli_runner, epic_with_questions):
    ...
    def test_list_with_json_output(self, cli_runner, epic_with_questions):
...
class TestQuestionShowCommand:
    ...
    @pytest.fixture
    def epic_with_question(self, temp_repo):
        """Create plan folder with a single question."""
        import time
        import yaml

        plan_path = temp_repo / "docs" / "epics" / "live" / "260203QC_show_test"
        plan_path.mkdir(parents=True)
    ...
    def test_show_question_details(self, cli_runner, epic_with_question):
        """Test showing question details."""
        result = cli_runner([
            "plan", "question", "show",
            "Q-20260203-120000-dddd",
            "--plan", str(epic_with_question)
        ])
    ...
    def test_show_with_json_output(self, cli_runner, epic_with_question):
    ...
    def test_show_nonexistent_question(self, cli_runner, epic_with_question):
...
class TestQuestionAnswerCommand:
    ...
    def plan_with_pending_question(self, temp_repo):
        ...
        plan_path = temp_repo / "docs" / "epics" / "live" / "260203QC_answer_test"
        plan_path.mkdir(parents=True)
...
class TestQuestionDeferCommand:
    ...
    @pytest.fixture
    def epic_with_question_to_defer(self, temp_repo):
        """Create plan folder with a question to defer."""
        import time
        import yaml

        plan_path = temp_repo / "docs" / "epics" / "live" / "260203QC_defer_test"
        plan_path.mkdir(parents=True)
    ...
    def test_defer_question(self, cli_runner, epic_with_question_to_defer):
        """Test deferring a question."""
        result = cli_runner([
            "agent", "question", "defer",
            "Q-20260203-140000-ffff",
            "--plan", str(epic_with_question_to_defer)
        ])
        ...
        import yaml
        pending_dir = epic_with_question_to_defer / "questions" / "pending"
    ...
    def test_defer_with_json_output(self, cli_runner, epic_with_question_to_defer):
    ...
    def test_defer_nonexistent_question(self, cli_runner, epic_with_question_to_defer):
