...
class TestTaskHelp:
    ...
    def test_plan_task_help(self, cli_runner):
        ...
        stdout, stderr, code = cli_runner(["plan", "task", "--help"])
        assert code == 0
...
class TestTaskList:
    ...
    def test_task_list_shows_tasks(self, task_cli_runner):
        ...
        stdout, stderr, code = task_cli_runner(["plan", "task", "list"])
        assert code == 0
...
class TestTaskUpdate:
    ...
    def test_task_update_help(self, cli_runner):
        ...
        stdout, stderr, code = cli_runner(["plan", "task", "update", "--help"])
        assert code == 0
        ...
        if current_result.stdout.strip() and current_result.stdout.strip() != "null":
            ...
            if current_data and current_data.get("id"):
                ...
                update_result = task_cli_runner(
                    ["plan", "task", "update", task_id, "--status", "completed"]
                )
...
class TestTaskPrefill:
    ...
    def test_task_prefill_loads_preset(self, task_cli_runner):
        ...
        stdout, stderr, code = task_cli_runner(
            ["plan", "task", "prefill", "--preset", "planner-build"]
        )
...
class TestTaskAdd:
    ...
    def test_task_add_creates_task(self, task_cli_runner):
        ...
        # Add a new task
        result = task_cli_runner(["plan", "task", "add", "New test task"])
            ...
            if isinstance(task_list, list) and task_list:
                ...
                if isinstance(first_task, dict) and first_task.get("id"):
                    ...
                    status_result = task_cli_runner(
                        ["plan", "task", "status", first_task["id"]]
                    )
                    # Should succeed
