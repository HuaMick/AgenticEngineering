...
class NtfyReplyPoller:
    """Polls an ntfy topic for reply messages and delegates to NtfyQuestionAgent.

    Runs in a daemon thread alongside the filesystem watcher. Periodically
    polls ntfy for new messages and delegates reply handling to the
    NtfyQuestionAgent which manages intent detection, answer mapping,
    and state tracking.
    """

    def __init__(self, plan_path: Path, topic: str, server: str = "https://ntfy.sh", poll_interval: int = 30):
        self._plan_path = plan_path
        self._topic = topic
        self._server = server
        self._poll_interval = poll_interval
        self._last_poll_time = str(int(time.time()))
        self._running = False
        self._thread = None
        self._processed_msg_ids: set[str] = set()
        # Create agent for reply handling
        from agenticcli.services.ntfy_question_agent import NtfyQuestionAgent
        self._agent = NtfyQuestionAgent(plan_path, topic, server)
    ...
    def _poll_loop(self):
        """Main loop: sleep, poll, process replies."""
        while self._running:
            ...
            try:
                ...
                logger.warning("ntfy poll error: %s", e)

    def _check_for_replies(self):
        """Poll ntfy and delegate replies to NtfyQuestionAgent."""
        from agenticcli.utils.ntfy import poll_ntfy

        messages = poll_ntfy(self._topic, since=self._last_poll_time, server=self._server)
        if not messages:
            return

        # Update last poll time to newest message time
        self._last_poll_time = str(max(m.get("time", 0) for m in messages))
        ...
        for msg in messages:
            ...
            # Skip outgoing notifications (sent by us)
            if text.startswith("Question ") or text.startswith("No more questions"):
                continue
            if text.startswith("Answer Received") or text.startswith("Answer saved"):
                continue

            # Delegate to agent
            try:
                self._agent.handle_reply(text)
            except Exception as e:
                logger.warning("Agent reply error: %s", e)
