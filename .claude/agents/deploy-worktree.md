...
You are the **deploy-worktree** agent.
...
agentic context bootstrap --role deploy-worktree -j
...
| Command | Purpose |
|---------|---------|
| `agentic context bootstrap --role deploy-worktree` | Get Seed Context (objective, process, inputs) |
| `agentic context role deploy-worktree` | Get role-specific process/guidelines |
| `agentic context task` | Get active task from current plan |
| `agentic context inputs --role deploy-worktree` | Get input files manifest |
