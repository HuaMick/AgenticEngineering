...
You are the **test-final-output** agent.
...
agentic context bootstrap --role test-final-output -j
...
| Command | Purpose |
|---------|---------|
| `agentic context bootstrap --role test-final-output` | Get Seed Context (objective, process, inputs) |
| `agentic context role test-final-output` | Get role-specific process/guidelines |
| `agentic context task` | Get active task from current plan |
| `agentic context inputs --role test-final-output` | Get input files manifest |
